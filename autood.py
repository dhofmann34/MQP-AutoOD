...
def mahalanobis(x):
    ...
    try:  # if matrix matrix is not invertible use pinv
        inv_covmat = sp.linalg.inv(
            cov)  # DH old line: inv_covmat = sp.linalg.inv(cov) ||| new line: inv_covmat = sp.linalg.pinv(cov)
    except:
...
def load_dataset(filename, index_col_name=None, label_col_name=None):
    if filename.endswith('arff'):
        ...
        data = data.rename(columns={index_col_name: 'id',
                                    label_col_name: 'label'})  # DH set all data to the same label and id col names
        index_col_name = 'id'
...
class AutoOD:
    ...
    def _run_unsupervised_outlier_detection_methods(self):
        ...
        if OutlierDetectionMethod.LOF in self.params.detection_methods:
            num_detectors += self._run_lof(all_results, all_scores, methods_to_best_f1, f1s,
                                           num_detectors, instance_index_ranges, detector_index_ranges)

        if OutlierDetectionMethod.KNN in self.params.detection_methods:
            num_detectors += self._run_knn(all_results, all_scores, methods_to_best_f1, f1s,
                                           num_detectors, instance_index_ranges, detector_index_ranges)

        if OutlierDetectionMethod.IsolationForest in self.params.detection_methods:
            num_detectors += self._run_isolation_forest(all_results, all_scores, methods_to_best_f1, f1s,
                                                        num_detectors, instance_index_ranges, detector_index_ranges)
    ...
    def autood_training(self, L, scores, X, y, instance_index_ranges,
        ...
        for i_range in range(0, 50):
            num_methods = np.shape(L)[
                1]  # L is matrix: data points x unsup methods, L[0] gives us the number of detectors
            agree_outlier_indexes = np.sum(L, axis=1) == np.shape(L)[
                1]  # index of all datapoints where they all agree on outlier
            ...
            all_outlier_indexes = np.union1d(np.where(agree_outlier_indexes)[0],
                                             prediction_high_conf_outliers)  # set with all outliers that agree
            all_inlier_indexes = np.setdiff1d(all_inlier_indexes,
                                              prediction_classifier_disagree)  # set with all inliers that agree

            self_agree_index_list = []
            if ((len(all_outlier_indexes) == 0) or (len(all_inlier_indexes) / len(
                    all_outlier_indexes) > 1000)):  # if no outliers in reliable set: loosen requirments
            ...
            all_outlier_indexes = np.union1d(all_outlier_indexes,
                                             self_agree_index_list)  # add empty set if outliers are in reliable set, else add self agree
            if (len(np.setdiff1d(all_outlier_indexes,
                                 prediction_classifier_disagree)) != 0):  # DH: remove reliable only if set will not be 0
            ...
            prediction_classifier_disagree = np.where(temp_prediction != temp_classifier)[
                0]  # gets index of where do SVM and log. disagree

            two_prediction_corr.append(np.corrcoef(clf_predict_proba, clf_predict_proba_X)[
                                           0, 1])  # Return Pearson product-moment correlation coefficients
            ...
            if np.max(coef_index_range) >= 2:
                if (len(prediction_high_conf_outliers) > 0 and len(prediction_high_conf_inliers) > 0):
                    ...
                    clf_prune_2 = LogisticRegression(random_state=0, penalty='l2', max_iter=max_iter).fit(
                        scores_transformed[new_data_indexes],
                        new_labels)  # train log with updated labels: this is to prune poor detectors
                ...
                if (np.max(coef_index_range) >= 2):
                    if (len(set(combined_coef)) > 1):
                        ...
                        cutoff = max(max(0, np.mean(combined_coef) - np.std(combined_coef)),
                                     min(combined_coef))  # cutoff to purne poor detectors

                        remain_indexes_after_cond = (
                                cur_clf_coef > cutoff)  # np.logical_and(cur_clf_coef > cutoff, abs(cur_clf_coef) > 0.01) 
                        remain_params_tracking = remain_params_tracking[
                            remain_indexes_after_cond]  # prune poor detectors
        ...
        if np.max(coef_index_range) >= 2:
            if (len(prediction_high_conf_outliers) > 0 and len(
                    prediction_high_conf_inliers) > 0):  # losen requirements
        ...
        for i_range in range(0, 50):
            ...
            self_agree_index_list = []
            if ((len(all_outlier_indexes) == 0) or (
                    len(all_inlier_indexes) / len(all_outlier_indexes) > 1000)):  # losen requirements
                for i in range(0, len(index_range)):
                    if (index_range[i, 1] - index_range[i, 0] <= 6):
                        continue
                    temp_index = disagree_indexes[np.where(
            ...
            if (len(np.setdiff1d(all_outlier_indexes,
                                 prediction_classifier_disagree)) != 0):  # remove reliable only if set will not be 0
                all_outlier_indexes = np.setdiff1d(all_outlier_indexes, prediction_classifier_disagree)
